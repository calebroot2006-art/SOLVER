...
fn run(variant: Variant) {
    ...
    assert_eq!(strategy.rows().len(), game.num_nodes());
...
#[test]
fn leduc_vanilla_matches_every_captured_checkpoint() { run(Variant::Vanilla); }
#[test]
fn leduc_plus_matches_captured_curve_and_fixed_budget() { run(Variant::Plus); }
#[test]
