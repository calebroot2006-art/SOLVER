...
use postflop::{
    Cfr, Game, NodeId, NodeKind, Real, SolveConfig, SolveError, SolverConfig, StopReason, Strategy,
    Variant, solve,
};
...
fn nan_terminal_fails_loudly_and_cannot_return_a_strategy() {
    ...
    let error = solve(&NanGame, &mut solver, &config, |_| {
        panic!("nonfinite values must not be progress")
    })
    .unwrap_err();
    assert!(
        matches!(error, SolveError::NonFinite { iteration: 1, .. }),
        "{error:?}"
    );
...
fn iteration_cap_and_target_are_distinct_and_measured() {
    ...
    for (target, expected) in [
        (0.0, StopReason::IterationCap),
        (100.0, StopReason::TargetReached),
    ] {
...
fn private_weights_and_strategy_rows_are_checked() {
    for weights in [
        ...
        assert!(matches!(
            Cfr::new(&kuhn::game().with_weights(weights), Variant::Vanilla),
...
fn configuration_failures_name_the_field() {
    ...
    for (old, new, field) in [
        ...
        (
            "target_pct_of_pot=0.5",
            "target_pct_of_pot=nan",
            "target_pct_of_pot",
        ),
        (
            "target_pct_of_pot=0.5",
            "target_pct_of_pot=inf",
            "target_pct_of_pot",
        ),
        (
            "target_pct_of_pot=0.5",
...
impl Game for Malformed {
    ...
    fn num_private_states(&self, player: usize) -> usize {
        self.inner.num_private_states(player)
    }
    fn initial_weights(&self, player: usize) -> &[Real] {
        self.inner.initial_weights(player)
    }
    fn compatible(&self, first: usize, second: usize) -> bool {
        self.inner.compatible(first, second)
    }
    fn chance_prob(&self, node: NodeId, outcome: usize) -> Real {
        self.inner.chance_prob(node, outcome)
    }
    fn chance_mask(&self, node: NodeId, outcome: usize, player: usize) -> &[Real] {
        self.inner.chance_mask(node, outcome, player)
    }
    fn terminal_values(&self, node: NodeId, player: usize, opp: &[Real], out: &mut [Real]) {
        self.inner.terminal_values(node, player, opp, out);
    }
    fn starting_pot(&self) -> Real {
        self.pot
    }
    fn info_label(&self, node: NodeId, player: usize, state: usize) -> String {
        self.inner.info_label(node, player, state)
    }
}
