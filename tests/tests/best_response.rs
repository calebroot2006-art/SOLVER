...
fn uniform_profiles_match_independent_published_constants() {
    for (game, nash, infosets) in [
        (kuhn::game(), 11.0 / 12.0, 12),
        (leduc::game(), 4.747222222222222, 936),
    ] {
        ...
        close(
            metrics.pct_of_pot,
            100.0 * metrics.average / game.starting_pot(),
        );
        close(
            metrics.nash_conv,
            metrics.pct_of_pot * game.starting_pot() / 50.0,
        );
        assert_eq!(game.legal_information_sets(), infosets);
        for player in 0..2 {
            close(
                metrics.br_value[player],
                oracle.best_response(&uniform, player),
            );
            close(
                expected_value(&game, &uniform, player).unwrap(),
                oracle.expected_value(&uniform, player),
            );
        }
    }
    let game = kuhn::game();
        ...
        if let NodeKind::Chance { num_outcomes } = game.kind(id) {
            for first in 0..6 {
                for second in 0..6 {
                    ...
                    let mass: f64 = (0..usize::from(num_outcomes))
                        .map(|board| {
                            game.chance_prob(id, board)
                                * game.chance_mask(id, board, 0)[first]
                                * game.chance_mask(id, board, 1)[second]
                        })
                        .sum();
                    close(mass, 1.0);
                }
