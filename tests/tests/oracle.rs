    ...
    for (id, row) in rows.iter_mut().enumerate() {
        if let NodeKind::Player { num_actions, .. } = game.kind(id as u32) {
            for (hand, probabilities) in row.chunks_mut(usize::from(num_actions)).enumerate() {
                ...
                for probability in probabilities { *probability /= sum; }
            }
        }
    }
...
fn explicit_histories_match_values_and_legal_best_responses_on_weighted_ranges() {
    let cases = [
        ...
        leduc::game(),
        leduc::game().with_weights([vec![0.0, 0.2, 2.0, 1.0, 0.0, 0.7], vec![0.4, 1.7, 0.0, 0.5, 3.0, 0.0]]),
    ];
    for game in cases {
    ...
    for game in [
        ...
        for variant in [Variant::Vanilla, Variant::Plus, Variant::Discounted { alpha: 1.5, beta: 0.0, gamma: 2.0 }] {
            ...
            for iterations in 1..=3 {
                ...
                for (expected, actual) in [(current, solver.current_strategy().unwrap().clone()), (average, solver.average_strategy(&game).unwrap())] {
                    for (id, row) in expected.rows().iter().enumerate() {
                        let NodeKind::Player { player, num_actions } = game.kind(id as u32) else { continue; };
                        for (entry, want) in row.iter().enumerate() {
                            let hand = entry / usize::from(num_actions);
                            if game.initial_weights(usize::from(player))[hand] == 0.0 || game.board(id as u32) == Some(hand) { continue; }
                            let got = actual.row(id as u32).unwrap()[entry];
                            assert!((got - want).abs() < 1e-12, "{variant:?} iteration={iterations} {} entry={entry}: {got} != {want}", game.info_label(id as u32, usize::from(player), hand));
                        }
                    }
                }
            }
