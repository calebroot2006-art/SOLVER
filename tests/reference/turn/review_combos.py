"""Record independent action values and single-row effects for every turn policy mismatch.

The output is the `per-combo-review.json` that `compare.py --review` checks: a reviewer adds
`review_category` and `review_reasoning` to each row, and the comparison fails while any row
over two percentage points has no reasoning.

Read `oracle.py`'s docstring first. Rows inside an exported runout carry independently
recomputed action values; rows in the turn round carry the capture's own reported
continuation value, and say so in `continuation_sources`.
"""
...
from compare import FREQUENCY_THRESHOLD, compare
...
def interpret(delta, project, reference):
    if max(delta) <= FREQUENCY_THRESHOLD:
        return "Frequency difference is within two percentage points after refinement."
    if reference["own_history_reach"] == 0:
        return (
            "Reference own history reach is exactly zero; its row is outside its profile path. "
            "Project reach, counterfactual action gaps and single-row effects are recorded "
            "separately."
        )
    if (
        project["compatible_opponent_mass"] == 0
        or reference["compatible_opponent_mass"] == 0
    ):
        return (
            "Compatible opposing reach is zero in at least one profile; that profile has no "
            "defined conditional action EV here."
        )
    if "reported_chance_node_ev" in set(project["continuation_sources"]) | set(
        reference["continuation_sources"]
    ):
        return (
            "This is a turn-round row: its continuation crosses runouts that were not "
            "exported, so the action values come from each capture's own reported chance-node "
            "value rather than from an independent recomputation. Numerical review required."
        )
    return (
        "Both profiles reach this information set inside an exported runout, and the action "
        "values are independently recomputed. Their action gaps and changes from the initial "
        "capture require numerical review; low root residual alone does not explain the row."
    )
...
def review(project, reference, initial_project, initial_reference):
    ...
    for case, baseline, pc, rc, pi, ri in zip(
        ...
        require(
            all(c["input"] == pc["input"] for c in (rc, pi, ri)), "Review cases differ"
        )
            ...
            details.append(
                {
                    ...
                    "interpretation": interpret(delta, pe, re),
                }
            )
        reports.append(
            {
                ...
                "remaining_frequency_rows": sum(
                    max(d["final_frequency_differences"]) > FREQUENCY_THRESHOLD
                    for d in details
                ),
                "rows": details,
            }
    ...
    return {
        ...
        "street": "turn",
        "scope": (
            "Every row differing by more than two percentage points in either the initial or "
            "the refined capture, over the exported runouts. Metrics evaluate normalized "
            "exported policies. Each row effect changes only that own information set, holding "
            "all other rows fixed; effects are not additive and are not bounds for other "
            "decisions."
        ),
