...
impl Combo {
    ...
    pub const fn new(first: Card, second: Card) -> Result<Self, CardError> {
        if first.id() == second.id() {
            return Err(CardError::DuplicateCard(first));
        }
        let cards = if first.id() < second.id() {
            [first, second]
        } else {
            [second, first]
        };
    ...
    pub fn from_id(id: u16) -> Result<Self, CardError> {
        if id >= 1326 {
            return Err(CardError::InvalidComboId(id));
        }
        let mut b = 1_u16;
        while b * (b + 1) / 2 <= id {
            b += 1;
        ...
        Ok(Self {
            id,
            cards: [Card::from_id(a as u8)?, Card::from_id(b as u8)?],
        })
    }
