...
impl Range {
    ...
    #[must_use]
    pub const fn weights(&self) -> &[f64; 1326] {
        &self.weights
    }
    ...
    pub fn without_cards(&self, dead: CardSet) -> Self {
        ...
        for combo in Combo::all() {
            if combo.mask() & dead.bits() != 0 {
                result.weights[combo.id() as usize] = 0.0;
            }
        }
        result
    }
    ...
    pub fn parse(text: &str) -> Result<Self, RangeError> {
        ...
        if text.bytes().all(is_range_whitespace) {
            return Ok(Self::empty());
        }
        let mut tokens = Vec::new();
        for (item, part) in text.split(',').enumerate() {
            if part.bytes().all(is_range_whitespace) {
                return Err(RangeError::EmptyCommaItem { item });
            }
            for token in part
                .split(|c: char| c.is_ascii() && is_range_whitespace(c as u8))
                .filter(|token| !token.is_empty())
            {
                tokens.push(token);
                if tokens.len() > MAX_RANGE_TOKENS {
                    return Err(RangeError::TooManyTokens {
                        count: tokens.len(),
                        limit: MAX_RANGE_TOKENS,
                    });
                }
            }
        }
        ...
        for token in tokens {
            let (expression, weight) = if let Some((expression, suffix)) = token.split_once(':') {
                let weight = suffix
                    .parse::<f64>()
                    .map_err(|_| invalid(token, "weight must be a number in [0,1]"))?;
...
pub fn combos_for_cell(row: usize, col: usize) -> Result<Vec<Combo>, RangeError> {
    if row >= 13 || col >= 13 {
        return Err(RangeError::InvalidCell { row, col });
    }
    Ok(Combo::all()
        .filter(|combo| combo.grid_cell() == (row, col))
        .collect())
}

fn is_range_whitespace(byte: u8) -> bool {
    // Include vertical tab as well as Rust's five ASCII whitespace bytes.
...
fn validate_weight(combo: Combo, weight: f64) -> Result<(), RangeError> {
    if weight.is_finite() && (0.0..=1.0).contains(&weight) {
        ...
        Err(RangeError::InvalidWeight { combo, weight })
    }
}

fn invalid(token: &str, reason: &str) -> RangeError {
    RangeError::InvalidToken {
        token: token.to_owned(),
...
impl Class {
    fn parse(text: &str) -> Result<Self, &'static str> {
        ...
        if !(2..=3).contains(&bytes.len()) {
            return Err("expected a two-rank class with optional s or o suffix");
        }
        let high = Rank::from_symbol(bytes[0])
            .ok_or("invalid high rank")?
            .index();
        let low = Rank::from_symbol(bytes[1])
            .ok_or("invalid low rank")?
            .index();
        if high < low {
            return Err("class ranks must be in descending order");
        }
        ...
        if high == low && kind != Kind::Both {
            return Err("pairs cannot have a suitedness suffix");
        }
    ...
    fn matches(self, combo: Combo) -> bool {
        ...
        a.rank().index() == self.low
            && b.rank().index() == self.high
            && match self.kind {
                Kind::Both => true,
                Kind::Suited => suited,
                Kind::Offsuit => !suited,
...
fn expand(expression: &str) -> Result<Vec<Combo>, &'static str> {
    let classes = if let Some((start, end)) = expression.split_once('-') {
        ...
        if start.kind != end.kind {
            return Err("interval suffixes must match");
        }
        let start_pair = start.high == start.low;
        let end_pair = end.high == end.low;
        if start_pair != end_pair {
            return Err("interval cannot mix a pair and a nonpair");
        }
        if start_pair {
            ...
            return Err("interval must keep its high rank or its rank gap constant");
        }
        ...
        return expression
            .parse::<Combo>()
            .map(|combo| vec![combo])
            .map_err(|_| "expected two distinct cards");
    } else {
        vec![Class::parse(expression)?]
    };
    Ok(Combo::all()
        .filter(|&combo| classes.iter().any(|class| class.matches(combo)))
        .collect())
}
