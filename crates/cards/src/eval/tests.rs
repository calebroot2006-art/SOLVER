...
fn correspondence() -> &'static [u32] {
    ...
    SCORES.get_or_init(|| {
                        ...
                        for e in d + 1..52 {
                            ...
                            if *slot == 0 {
                                *slot = expected;
                                ...
                                assert_eq!(*slot, expected, "incorrect tie: {cards:?}");
                            }
                        }
                    }
                }
        ...
        eprintln!(
            "five_card_exhaustive hands=2598960 classes={distinct} elapsed_seconds={:.3}",
    ...
    fn hand(&mut self) -> [Card; 7] {
        ...
        std::array::from_fn(|_| {
            loop {
                ...
                if value < 52_u64.wrapping_neg() % 52 {
                    continue;
                }
...
fn seven_cards_match_ten_million_independent_subset_oracles() {
    ...
    for sample in 1..=10_000_000 {
        ...
        assert_eq!(
            scores[usize::from(actual.0.to_raw())],
            expected,
            "sample={sample}, cards={cards:?}"
        );
        if sample == 100_000 || sample % 1_000_000 == 0 {
            eprintln!(
                "seven_card_oracle samples={sample} elapsed_seconds={:.3}",
                started.elapsed().as_secs_f64()
            );
        }
    }
...
fn special_hands_and_full_flush_kickers_are_ordered() {
    ...
    for (text, category) in cases {
        ...
        assert_eq!(
            correspondence()[usize::from(actual.0.to_raw())],
            oracle::seven(cards)
        );
    }
    assert!(
        evaluate_seven(hand("AsKs9s7s3s2d4h")).unwrap()
            > evaluate_seven(hand("AsQs9s7s3s2d4h")).unwrap()
    );
    assert!(
        evaluate_seven(hand("AsAhAdKsKh2d4h")).unwrap()
            > evaluate_seven(hand("AsKs9s7s3s2d4h")).unwrap()
    );
    assert!(
        evaluate_seven(hand("As2d3h4c5sKdQh")).unwrap()
            < evaluate_seven(hand("2d3h4c5s6dKdQh")).unwrap()
    );
}
        ...
        for a in 0..4 {
            for b in 0..4 {
                ...
                for c in 0..4 {
                    ...
                    let changed = cards.map(|card| {
                        Card::new(
                            card.rank(),
                            Suit::from_index(permutation[usize::from(card.suit().index())])
                                .unwrap(),
                        )
                    });
...
fn cached_board_matches_direct_evaluation_and_rejects_overlap() {
    for text in [
        ...
        for combo in Combo::all().filter(|combo| combo.mask() & dead.bits() == 0) {
            let [a, b] = combo.cards();
            assert_eq!(
                evaluator.evaluate(combo).unwrap(),
                evaluate_seven([board[0], board[1], board[2], board[3], board[4], a, b]).unwrap()
            );
        }
    }
    ...
    assert_eq!(
        royal.evaluate("2c3d".parse().unwrap()).unwrap(),
        royal.evaluate("AhAd".parse().unwrap()).unwrap()
    );
}
...
fn report_checked_evaluation_workload_timings() {
    ...
    let board = hand::<5>("AcKd7s4h2c");
    let dead = CardSet::new(&board).unwrap();
    let combos: Vec<_> = Combo::all()
        .filter(|combo| combo.mask() & dead.bits() == 0)
        .collect();
    ...
    for _ in 0..100 {
        for combo in &combos {
            black_box(evaluator.evaluate(black_box(*combo)).unwrap());
        }
    }
    ...
    for _ in 0..100 {
        for combo in &combos {
            black_box(evaluate_holdem(black_box(board), black_box(*combo)).unwrap());
        }
    }
    eprintln!(
        "evaluation_timing os={} arch={} profile=test_opt2 checked_seven_hands=100000 median_seconds={:.6} board_setup_seconds={setup_seconds:.9} cached_hands={} cached_seconds={cached_seconds:.6} rebuilt_board_seconds={:.6} backend_static_bytes=312320 river_evaluator_bytes={}",
        std::env::consts::OS,
        std::env::consts::ARCH,
        durations[2],
        100 * combos.len(),
        start.elapsed().as_secs_f64(),
        std::mem::size_of::<RiverEvaluator>()
    );
}
