use std::fmt;
use crate::{Cfr, Exploitability, NodeId, Progress, SolveConfig, SolveError, SolveReport, Variant,
    allocation::reserved, best_response::exploitability_bound,
    solver::{SolveSession, drive}, terminal::ShowdownScratch};
...
impl fmt::Debug for RiverSolver {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RiverSolver").field("game", &self.game).field("iteration", &self.iteration()).finish()
    }
}
...
impl RiverSolver {
    ...
    pub fn new(game: RiverGame, variant: Variant) -> Result<Self, SolveError> {
        ...
        let lease = game.inner.budget.reserve(memory.solver_bytes + memory.scratch_bytes)?;
        ...
        Ok(Self { game, core, scratch, _lease: lease })
    }
    ...
    pub fn iteration(&self) -> u64 { self.core.iteration() }
    ...
    pub fn run_iteration(&mut self) -> Result<(), SolveError> {
        let _workspace = self.game.inner.budget.reserve(self.game.inner.memory.traversal_bytes)?;
        self.core.advance(&mut RiverTerminal { game: &self.game.inner, scratch: &mut self.scratch[0] })
    }
    /// Retain the reach-weighted average and its exact game binding.
    pub fn average_strategy(&self) -> Result<RiverStrategy, SolveError> {
        let lease = self.game.inner.budget.reserve(self.game.inner.memory.snapshot_bytes)?;
        let policy = self.core.average_bound()?;
        Ok(RiverStrategy { game: self.game.clone(), policy, _lease: lease })
    }
    ...
    pub fn solve(&mut self, config: &SolveConfig, on_progress: impl FnMut(&Progress)) -> Result<SolveReport, SolveError> {
    ...
    pub fn solve_with_cancel(&mut self, config: &SolveConfig, on_progress: impl FnMut(&Progress), should_cancel: impl FnMut() -> bool) -> Result<SolveReport, SolveError> {
...
impl SolveSession for RiverSolver {
    ...
    fn step(&mut self) -> Result<(), SolveError> { self.run_iteration() }
    fn measurement(&mut self) -> Result<Exploitability, SolveError> {
        let strategy = self.average_strategy()?;
        let _workspace = self.game.inner.budget.reserve(self.game.inner.memory.traversal_bytes)?;
        exploitability_bound(&mut RiverTerminal { game: &self.game.inner, scratch: &mut self.scratch[0] }, &strategy.policy)
    }
}
