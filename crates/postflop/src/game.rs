...
impl Layout {
    pub fn new(game: &dyn Game) -> Result<Self, SolveError> {
        ...
        if count == 0 || count > NodeId::MAX as usize || game.root() as usize >= count {
            return Err(invalid("node count and root must describe a nonempty indexed tree"));
        }
        let states = [game.num_private_states(0), game.num_private_states(1)];
        if states.contains(&0) { return Err(SolveError::EmptyGame); }
        states[0].checked_mul(states[1]).ok_or_else(|| invalid("private-pair allocation overflow"))?;
        let weights = [game.initial_weights(0).to_vec(), game.initial_weights(1).to_vec()];
        for player in 0..2 {
            if weights[player].len() != states[player] || weights[player].iter().any(|v| !v.is_finite() || *v < 0.0) {
                return Err(invalid("initial_weights must match private-state counts and be finite and nonnegative"));
            }
        }
        ...
        for (h0, w0) in weights[0].iter().enumerate() {
            for (h1, w1) in weights[1].iter().enumerate() {
                ...
                if legal { normalizer += w0 * w1; }
            }
        }
        if !normalizer.is_finite() || normalizer <= 0.0 { return Err(SolveError::EmptyGame); }
        let pot = game.starting_pot();
        if !pot.is_finite() || pot <= 0.0 { return Err(invalid("starting_pot must be finite and positive")); }
        let mut nodes = Vec::with_capacity(count);
        for id in 0..count {
            let kind = game.kind(id as NodeId);
            let n = match kind {
                NodeKind::Player { player, num_actions } => {
                    if player > 1 || num_actions == 0 { return Err(invalid("player nodes require player 0 or 1 and nonzero actions")); }
                    states[player as usize].checked_mul(num_actions as usize).ok_or_else(|| invalid("strategy allocation overflow"))?;
                    num_actions as usize
                }
                NodeKind::Chance { num_outcomes } => {
                    if num_outcomes == 0 { return Err(invalid("chance node has no outcomes")); }
                    num_outcomes as usize
                }
                NodeKind::Terminal => 0,
            };
            let children: Vec<_> = (0..n).map(|i| game.child(id as NodeId, i)).collect();
            if children.iter().any(|child| *child as usize >= count) { return Err(invalid("child index outside node storage")); }
            let mut probabilities = Vec::new();
            let mut masks = Vec::new();
            if matches!(kind, NodeKind::Chance { .. }) {
                for outcome in 0..n {
                    let p = game.chance_prob(id as NodeId, outcome);
                    if !p.is_finite() || !(0.0..=1.0).contains(&p) { return Err(invalid("chance_prob must be finite and in [0,1]")); }
                    let pair = [game.chance_mask(id as NodeId, outcome, 0).to_vec(), game.chance_mask(id as NodeId, outcome, 1).to_vec()];
                    for player in 0..2 {
                        if pair[player].len() != states[player] || pair[player].iter().any(|v| *v != 0.0 && *v != 1.0) {
                            return Err(invalid("chance_mask must contain a zero-or-one entry per private state"));
                        }
                    }
    ...
    fn validate_paths(&self) -> Result<(), SolveError> {
        ...
        let mut stack = vec![(self.root, [vec![1.0; self.states[0]], vec![1.0; self.states[1]]], 0usize)];
        while let Some((id, live, depth)) = stack.pop() {
            if visited[id as usize] { return Err(SolveError::InvalidGame("public nodes must form a tree without cycles or shared children".into())); }
            ...
            if matches!(node.kind, NodeKind::Chance { .. }) {
                for h0 in 0..self.states[0] {
                    for h1 in 0..self.states[1] {
                        if live[0][h0] == 0.0 || live[1][h1] == 0.0 || !self.compatible[h0 * self.states[1] + h1] { continue; }
                        let mass: Real = node.probabilities.iter().zip(&node.masks).map(|(p, mask)| p * mask[0][h0] * mask[1][h1]).sum();
                        if (mass - 1.0).abs() > 1e-12 {
                            return Err(SolveError::InvalidGame(format!("chance mass at node {id} for ({h0},{h1}) is {mass}, expected one")));
                        }
                    }
                }
            }
            for (outcome, child) in node.children.iter().enumerate() {
                ...
                if matches!(node.kind, NodeKind::Chance { .. }) {
                    for (player, entries) in next_live.iter_mut().enumerate() {
                        for (h, entry) in entries.iter_mut().enumerate() { *entry *= node.masks[outcome][player][h]; }
                    }
                }
                stack.push((*child, next_live, depth + 1));
            }
        }
        if visited.contains(&false) { return Err(SolveError::InvalidGame("node storage contains unreachable nodes".into())); }
        Ok(())
    }

    pub fn row_len(&self, node: usize) -> usize {
        match self.nodes[node].kind {
            NodeKind::Player { player, num_actions } => self.states[player as usize] * num_actions as usize,
            _ => 0,
        }
    }

    pub fn check_game(&self, game: &dyn Game) -> Result<(), SolveError> {
        let changed = || SolveError::InvalidGame("game differs from the strategy/solver binding".into());
        if game.root() != self.root || game.num_nodes() != self.nodes.len() || game.starting_pot() != self.pot {
            return Err(changed());
        }
        for player in 0..2 {
            if game.num_private_states(player) != self.states[player] || game.initial_weights(player) != self.weights[player] { return Err(changed()); }
        }
        for h0 in 0..self.states[0] {
            for h1 in 0..self.states[1] {
                if game.compatible(h0,h1) != self.compatible[h0 * self.states[1] + h1] { return Err(changed()); }
            }
        }
        for (id, node) in self.nodes.iter().enumerate() {
            if game.kind(id as NodeId) != node.kind { return Err(changed()); }
            for (action, child) in node.children.iter().enumerate() {
                if game.child(id as NodeId, action) != *child { return Err(changed()); }
            }
            for (outcome, p) in node.probabilities.iter().enumerate() {
                if game.chance_prob(id as NodeId, outcome) != *p { return Err(changed()); }
                for player in 0..2 {
