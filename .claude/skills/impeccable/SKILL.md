...
1. **[R&R install note — docs-only]** This repo installs Impeccable without its script engine, subagents, or hooks: no `scripts/` folder exists, so skip every `node .../scripts/*.mjs` and `npx impeccable` invocation this skill or its references mention (context.mjs, hook.mjs, pin.mjs, serve-question.mjs, the `live` command's tooling, `/impeccable hooks`, `/impeccable doctor`). Work directly from the reference playbooks; where a reference offers a "degraded" variant (reference/degraded/), prefer it. Read PRODUCT.md and DESIGN.md yourself if they exist in the project.
